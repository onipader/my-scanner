...
with st.sidebar:
    ...
    top_n = st.number_input("스캔할 종목 수 (시총순)", min_value=1, max_value=2000, value=10) # 10개 설정 가능
...
if start_button:
    ...
    try:
        # 1. 종목 리스트 확보 및 시총순 필터링 (가장 중요한 수정 구간)
        tickers = []
        if "국내" in market:
            ...
            # 여기서 사용자가 설정한 top_n 만큼만 정확히 자릅니다.
            ...
            tickers = [(m['market'], m['korean_name']) for m in raw_tickers[:int(top_n)]]

        # 2. 분석 실행 (이제 tickers 리스트는 정확히 top_n개 입니다)
        actual_scan_count = len(tickers)
        for i, (symbol, name) in enumerate(tickers):
            progress_bar.progress((i + 1) / actual_scan_count)
            status_text.text(f"분석 중: {name} ({i+1}/{actual_scan_count})")
        ...
        status_text.text(f"✅ 분석 완료! ({actual_scan_count}개 종목 스캔 완료)")
...
if st.session_state.found_data:
    ...
    st.download_button("📥 결과 저장", csv, "signals.csv", "text/csv")
elif start_button:
