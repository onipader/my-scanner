...
with st.sidebar:
    ...
    top_n = st.slider("시가총액 상위 스캔 범위", 50, 500, 100, step=50)
    
    tf_map = {"월봉": "1M", "주봉": "1W", "일봉": "", "4시간봉": "240"}
    st.info("차트 바늘이 'BUY' 이상인 종목만 정밀 추출합니다.")
...
if start_btn:
    ...
    for item in raw:
        ...
        score = d[0]
        # 🔹 핵심 수정: 점수 기준을 0.2로 높여서 차트의 'Neutral'을 걸러냅니다.
        if score >= 0.2: 
            results.append({
                "종목명": d[3],
    ...
    if results:
        ...
        st.success(f"차트 신호와 100% 일치하는 {len(results)}개 종목 포착!")
    else:
        st.warning("현재 차트 바늘이 'BUY'를 가리키는 우량주가 없습니다.")
