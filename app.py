...
st.title("📈 차트 신호 직결: Double BB 스캐너 (최종형)")
...
with st.sidebar:
    ...
    # 🔹 신호 감도: -1.0(모두 포착) ~ 1.0(엄격함). 비트코인을 위해 -0.5 추천
    sensitivity = st.slider("신호 감도 (낮을수록 잘 잡힘)", -1.0, 1.0, -0.5, step=0.1)
...
def get_direct_signal(symbol, screener, exchange, interval):
    try:
        ...
        # Recommend.All: 트레이딩뷰 엔진이 모든 지표를 종합해 내린 BUY/SELL 점수
        payload = {
        ...
        # 🔹 판정 로직: 추천 점수가 감도보다 높거나, 현재가가 하단선 근처인 경우
        if rec_val > sensitivity or curr_c <= bb_low * 1.05:
...
if start_button:
    ...
    for i, (sym, name, exch, scr) in enumerate(tickers[:top_n]):
        ...
        if res:
            ...
            found_list.append({
                ...
                "신호점수": round(res['score'], 2), "하단선(참고)": f"{res['bb_low']:,.0f}원"
            })
        time.sleep(0.01)
    ...
    if found_list:
        ...
        st.warning("신호가 잡히지 않습니다. 감도를 더 낮춰보세요.")
