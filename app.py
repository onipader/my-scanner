...
st.title("💰 글로벌 주식 & 코인 매수신호 스캐너 (개선판)")
st.markdown("전 세계 주식과 업비트 코인을 분석하여 **볼린저 밴드 하단 돌파 + 장기 상승 추세(EMA 200)** 종목을 찾습니다.")
...
def check_signal(close_series):
    """개선된 매수 신호 계산기 (볼린저 밴드 + EMA 200 필터 적용)"""
    # EMA 200일선 계산을 위해 최소 200개의 데이터가 필요함
    if len(close_series) < 200: return None 
    
    # 1. 볼린저 밴드 (20일 기준, 표준편차 2)
    ...
    # [조건] 밴드 하단 돌파(반등) + 현재 주가가 200일선 위에 있는 상승 추세 종목만!
...
# --- 분석 로직 시작 ---
if start_button:
    st.session_state.found_data = [] # 새로운 분석 시 데이터 초기화
    ...
    if "업비트" in market:
        ...
        for i, t in enumerate(tickers):
            ...
            try:
                # 데이터 수집 (충분한 데이터 확보를 위해 기간을 넉넉히 잡음)
                data = yf.download(t, interval=inter, period=per, progress=False)
                if len(data) < 200: continue # 데이터 부족 시 제외
...
if st.session_state.found_data:
    ...
    st.warning("추세 필터를 통과한 신호가 발견되지 않았습니다. 조건을 만족하는 종목이 나올 때까지 기다려보세요.")